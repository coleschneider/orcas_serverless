...
export const run = asyncMiddleware(async event => {
  const body = JSON.parse(event.body as string);

  if (!body || !body.email_address) throw new BadRequest('Email is required');

  if (!isOffline && isProd) {
    // We only want to make a request to apichimp if we're in prod

    const memberBody = { members: [generateMemberSubs(body)] };
    await serviceInstance.post(`/lists/${listId}`, memberBody);
  }
  return {
    // Don't catch other service errors - otherwise anyone will know an email
